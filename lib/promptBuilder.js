...
const { buildHumanTextureGuidanceBlock } = require('./humanTextureEngine');
const { buildKeywordPsychologyGuidanceBlock } = require('./keywordPsychologyEngine');
...
const plannerEditorialKnowledgeFileNames = [
  'editorial-rules.md',
  'proven-patterns.md',
];
...
async function loadEditorialMemory() {
  ...
  return Promise.all(
    plannerEditorialKnowledgeFileNames.map((fileName) =>
      loadMarkdownFile(path.join(editorialDirectory, fileName)),
    ),
  );
}
...
async function buildPrompt({
  ...
  if (articleBrief !== undefined) {
    if (typeof articleBrief !== 'string' || !articleBrief.trim()) {
      throw new Error('Writer prompts require a completed Article Brief.');
    }
    sectionsToJoin.push(`# Approved Article Brief\n\n${articleBrief}`);
  }

  return sectionsToJoin.join('\n\n');
}

/**
 * Builds a planning prompt from a topic. The planner creates title candidates;
 * it does not receive or select a final title.
 *
 * @param {object} options - Planning input values.
 * @param {string} options.series - The selected content series.
 * @param {string} options.topic - The subject to plan.
 * @param {string} options.strategyEvidence - Evidence supplied for the editorial decision.
 * @returns {Promise<string>} The assembled planning prompt.
 */
async function buildPlanningPrompt({ series, topic, strategyEvidence }) {
  if (typeof strategyEvidence !== 'string' || !strategyEvidence.trim()) {
    throw new Error('Planning requires non-empty Strategy Evidence.');
  }
  const [sharedSections, editorialMemory, keywordPsychologyGuidance, humanTextureGuidance, task] = await Promise.all([
    loadSharedPromptSections(series),
    loadEditorialMemory(),
    buildKeywordPsychologyGuidanceBlock(),
    buildHumanTextureGuidanceBlock(),
    loadMarkdownFile(path.join(projectRoot, 'prompts', 'tasks', 'article-plan.md')),
  ]);

  return [...sharedSections, ...editorialMemory, keywordPsychologyGuidance, humanTextureGuidance, task, `# Strategy Evidence Input\n\n${strategyEvidence.trim()}`, `# User Input\n\nTopic: ${topic}`]
    .join('\n\n');
}
 ...
 */
async function buildPublishingPrompts({ series, title, articleBrief }) {
  if (typeof articleBrief !== 'string' || !articleBrief.trim()) {
    throw new Error('Publishing prompts require an approved Article Brief.');
  }
  ...
  return Promise.all(
    promptTypes.map(async ({ filename, taskFile }) => ({
      ...
      content: await buildPrompt({
        ...
        sharedSections,
        taskFile,
        articleBrief: ['article.md', 'seo.md', 'seo-package.md', 'tags.md'].includes(taskFile)
          ? articleBrief
          : undefined,
      }),
    })),
...
module.exports = {
  plannerEditorialKnowledgeFileNames,
  buildPrompt,
  buildPlanningPrompt,
